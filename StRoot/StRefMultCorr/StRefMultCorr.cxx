    ...
    const Double_t zdcCoincidenceRate, const UInt_t flag) const
{
  ...
  if(mName.CompareTo("grefmult_P16id", TString::kIgnoreCase) == 0) correction_luminosity = (par0l==0.0) ? correction_luminosity : correction_luminosity*(par0l+par1l*30)/par0l; // Run14, P16id, refMult at ZdcX=30, other is at ZdcX=0;
...
void StRefMultCorr::readBadRuns()
{
  ...
  for(Int_t i=0; i<5; i++) {
    ...
    if(mName.CompareTo("grefmult_P16id", TString::kIgnoreCase) == 0 && year==2014) sprintf(inputFileName,"StRoot/StRefMultCorr/bad_runs_refmult_year%d_P16id.txt",year);
